...
_EXCLUDE_DIRS = {
    Path(".gitlab/runtime"),
    Path("src/python/tests/data"),
    Path("src/python/tests/utils/test_rloader"),
}
...
def check_header(header_template: PathLike) -> None:
    ...
    for file_path in _ROOT_PATH.rglob("*.*"):
        if file_path.is_file() and (file_path.suffix[1:] in _SUFFIXES_WITH_HEADER):
            if _EXCLUDE_DIRS.intersection(set(file_path.relative_to(_ROOT_PATH).parents)):
